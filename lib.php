...
/**
 * Library functions for local_aceengine.
 *
 ...
 */
function local_aceengine_is_pro_installed(): bool {
 ...
 */
function local_aceengine_is_pro_active(): bool {
    return local_aceengine_is_pro_installed()
        && \local_aceengine\licensing\license_manager::is_valid();
}
 ...
 */
function local_aceengine_is_enabled_for_course(int $courseid): bool {
    ...
    return (bool) get_config('local_aceengine', 'ace_course_enabled_' . $courseid);
}
 ...
 */
function local_aceengine_extend_navigation_course(navigation_node $parentnode, stdClass $course, context_course $context) {
    ...
    if (has_capability('local/ace:viewdashboard', $context)) {
        ...
        $parentnode->add(
            get_string('dashboard', 'local_aceengine'),
            $url,
            navigation_node::TYPE_CUSTOM,
            null,
            'local_aceengine_dashboard',
            new pix_icon('icon', get_string('pluginname', 'local_aceengine'), 'local_aceengine')
        );
    }

    if (has_capability('local/ace:viewanalytics', $context)) {
        $url = new moodle_url('/local/aceengine/analytics.php', ['courseid' => $course->id]);
        $parentnode->add(
            get_string('analytics', 'local_aceengine'),
            $url,
            navigation_node::TYPE_CUSTOM,
            null,
            'local_aceengine_analytics',
            new pix_icon('icon', get_string('analytics', 'local_aceengine'), 'local_aceengine')
        );
    }
}
 ...
 */
function local_aceengine_extend_settings_navigation(settings_navigation $settingsnav, context $context) {
 ...
 */
function local_aceengine_myprofile_navigation(\core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    if (!$iscurrentuser) {
        return;
    }
    ...
    $url = new moodle_url('/local/aceengine/my_quests.php');
    $node = new \core_user\output\myprofile\node(
        'miscellaneous',
        'local_aceengine_myquests',
        get_string('myquests', 'local_aceengine'),
        null,
        $url
    );
    $tree->add_node($node);
 ...
 */
function local_aceengine_extend_navigation_user_settings(navigation_node $parentnode, $user, $usercontext, $course, $coursecontext) {
    global $USER;
    ...
    $parentnode->add(
        ...
        null,
        'local_aceengine_myquests',
        new pix_icon('icon', get_string('myquests', 'local_aceengine'), 'local_aceengine')
    );
}
